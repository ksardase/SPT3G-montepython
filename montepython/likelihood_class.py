...
class Likelihood_spt3g(Likelihood):
	...
	def __init__(self, path, data, command_line):
		...
		self.need_cosmo_arguments(data, {'l_max_scalars': self.SPT3G_windows_lmax+1})
		
		self.ells = np.zeros(self.SPT3G_windows_lmax+1,'float64')
		for i in range(self.SPT3G_windows_lmax+1):
			self.ells[i] = i +1
		#print("self.ells=",self.ells)
		
		#Number of frequency bands
		N_freq_0 = 3
		#Maximum (uncropped) number of bandpowers
		self.N_b_0 = 44
		#Maximum (uncropped) number of spectra that can be used for parameter estimation
		self.N_s_0 = 12
		
		#spectra_to_fit_list_string = self.SPT3G_Y1_EETE_spectra_to_fit
		#print("spectra_to_fit_list_string=",spectra_to_fit_list_string)
		
		#Read in bandpowers
		#N_b x N_s matrix
		self.full_bandpowers = np.zeros((self.N_b_0, self.N_s_0), 'float64')
		datafile = open(os.path.join(self.data_directory, self.bandpower_filename), 'r')
		for i in range(self.N_b_0):
			line = datafile.readline()
			while line.find('#') != -1:
				line = datafile.readline()
			for j in range(self.N_s_0+1):
				if j!=0:
					self.full_bandpowers[i,j-1] = float(line.split()[j])
		datafile.close()
		#print(self.full_bandpowers)
		
		#Read in the covariance matrix
		#N_b*N_s x N_b*N_s matrix
		self.bdp_covariance = np.zeros((self.N_b_0*self.N_s_0, self.N_b_0*self.N_s_0), 'float64')
		datafile = open(os.path.join(self.data_directory, self.covariance_filename), 'r')
		for i in range(self.N_b_0*self.N_s_0):
			line = datafile.readline()
			while line.find('#') != -1:
				line = datafile.readline()
			for j in range(self.N_b_0*self.N_s_0):
				self.bdp_covariance[i,j] = float(line.split()[j])
		datafile.close()
		#print(self.bdp_covariance[0,:],self.bdp_covariance[-1,:])
		
		#Read in the beam covariance matrix
		#N_b*N_s x N_b*N_s matrix
		self.beam_covariance = np.zeros((self.N_b_0*self.N_s_0, self.N_b_0*self.N_s_0), 'float64')
		datafile = open(os.path.join(self.data_directory, self.beam_covariance_filename), 'r')
		for i in range(self.N_b_0*self.N_s_0):
			line = datafile.readline()
			while line.find('#') != -1:
				line = datafile.readline()
			for j in range(self.N_b_0*self.N_s_0):
				self.beam_covariance[i,j] = float(line.split()[j])
		datafile.close()
		self.beam_covariance *= self.BeamCovScaling
		#print(self.beam_covariance[0,:],self.beam_covariance[-1,:])
		
		#Read in the cal covariance matrix
		#N_freq_0*2 x N_freq_0*2
		self.full_cal_covariance_matrix = np.zeros((N_freq_0*2, N_freq_0*2), 'float64')
		datafile = open(os.path.join(self.data_directory, self.cal_covariance_filename), 'r')
		for i in range(N_freq_0*2):
			line = datafile.readline()
			while line.find('#') != -1:
				line = datafile.readline()
			for j in range(N_freq_0*2):
				self.full_cal_covariance_matrix[i,j] = float(line.split()[j])
		datafile.close()
		#print(self.full_cal_covariance_matrix)
		self.cal_inv_covariance = np.linalg.inv(self.full_cal_covariance_matrix)
		
		#Read in windows
		#N_l x N_s matrix for each bin
		self.full_windows = np.zeros((1+self.SPT3G_windows_lmax-self.SPT3G_windows_lmin,self.N_b_0,self.N_s_0), 'float64')
		#+1 more specs than needed because of ells in the file
		for k in range(self.N_b_0):
			i_s=repr(k+1)
			datafile = open(os.path.join(self.data_directory,self.window_folder,'window_' + i_s + '.txt'), 'r')
			for i in range(1+self.SPT3G_windows_lmax-self.SPT3G_windows_lmin):
				line = datafile.readline()
				while line.find('#') != -1:
					line = datafile.readline()
				for j in range(self.N_s_0+1):
					if j!=0:
						self.full_windows[i,k,j-1] = float(line.split()[j])
			datafile.close()
		#print(self.full_windows[:,43,:])
		
		#Read in the central frequencies of all bands
		#N_freq_0 x 1 Matrix
		self.full_nu_eff = np.zeros(N_freq_0, 'float64')
		datafile = open(os.path.join(self.data_directory, self.nu_eff_filename), 'r')
		for i in range(N_freq_0):
			line = datafile.readline()
			while line.find('#') != -1:
				line = datafile.readline()
			self.full_nu_eff[i] = float(line.split()[0])
		datafile.close()
		#print(self.full_nu_eff)
		
		#Loop over all spectra and grab their effective frequencies
		#N_s x 2 Matrix
		self.spectra_to_fit_nu_eff = np.zeros((self.N_s_0,2), 'float64')
		self.spectra_to_fit_nu_eff[0,0] = self.full_nu_eff[0]
		self.spectra_to_fit_nu_eff[1,0] = self.full_nu_eff[0]
		self.spectra_to_fit_nu_eff[2,0] = self.full_nu_eff[0]
		self.spectra_to_fit_nu_eff[3,0] = self.full_nu_eff[0]
		self.spectra_to_fit_nu_eff[4,0] = self.full_nu_eff[0]
		self.spectra_to_fit_nu_eff[5,0] = self.full_nu_eff[0]
		self.spectra_to_fit_nu_eff[6,0] = self.full_nu_eff[1]
		self.spectra_to_fit_nu_eff[7,0] = self.full_nu_eff[1]
		self.spectra_to_fit_nu_eff[8,0] = self.full_nu_eff[1]
		self.spectra_to_fit_nu_eff[9,0] = self.full_nu_eff[1]
		self.spectra_to_fit_nu_eff[10,0] = self.full_nu_eff[2]
		self.spectra_to_fit_nu_eff[11,0] = self.full_nu_eff[2]
		self.spectra_to_fit_nu_eff[0,1] = self.full_nu_eff[0]
		self.spectra_to_fit_nu_eff[1,1] = self.full_nu_eff[0]
		self.spectra_to_fit_nu_eff[2,1] = self.full_nu_eff[1]
		self.spectra_to_fit_nu_eff[3,1] = self.full_nu_eff[1]
		self.spectra_to_fit_nu_eff[4,1] = self.full_nu_eff[2]
		self.spectra_to_fit_nu_eff[5,1] = self.full_nu_eff[2]
		self.spectra_to_fit_nu_eff[6,1] = self.full_nu_eff[1]
		self.spectra_to_fit_nu_eff[7,1] = self.full_nu_eff[1]
		self.spectra_to_fit_nu_eff[8,1] = self.full_nu_eff[2]
		self.spectra_to_fit_nu_eff[9,1] = self.full_nu_eff[2]
		self.spectra_to_fit_nu_eff[10,1] = self.full_nu_eff[2]
		self.spectra_to_fit_nu_eff[11,1] = self.full_nu_eff[2]
		#print(self.spectra_to_fit_nu_eff[:,0],self.spectra_to_fit_nu_eff[:,1])
		print(" Fitting spectra:",'90_Ex90_E','90_Tx90_E','90_Ex150_E','90_Tx150_E','90_Ex220_E','90_Tx220_E','150_Ex150_E','150_Tx150_E','150_Ex220_E','150_Tx220_E','220_Ex220_E','220_Tx220_E')
		self.bin_min = self.bin_min - 1
		print("Over the bin range:",self.bin_min,"-",self.bin_max)
	...
	def loglkl(self, cosmo, data):
		...
		def DustFreqScaling(beta,Tdust,nu0,nu_eff):
			...
			fdust = fdust*Bnu(nu_eff,nu0,Tdust)/dBdT(nu_eff,nu0,cosmo.T_cmb())
			return fdust
			
				#Planck function normalised to 1 at nu0
		def Bnu(nu,nu0,T):
			Bnu = (nu/nu0)**3
			Bnu = Bnu * (np.exp( self.Ghz_Kelvin*nu0/T)-1.) / (np.exp( self.Ghz_Kelvin*nu/T)-1.)
			return Bnu
			
		#Derivative of Planck function normalised to 1 at nu0
		def dBdT(nu,nu0,T):
			x0 = self.Ghz_Kelvin*nu0/T
			x = self.Ghz_Kelvin*nu/T
		...
		Dl_Poisson_90x90 = data.mcmc_parameters['Dl_Poisson_90x90']['current']*data.mcmc_parameters['Dl_Poisson_90x90']['scale']
		Dl_Poisson_90x150 = data.mcmc_parameters['Dl_Poisson_90x150']['current']*data.mcmc_parameters['Dl_Poisson_90x150']['scale']
		Dl_Poisson_90x220 = data.mcmc_parameters['Dl_Poisson_90x220']['current']*data.mcmc_parameters['Dl_Poisson_90x220']['scale']
		Dl_Poisson_150x150 = data.mcmc_parameters['Dl_Poisson_150x150']['current']*data.mcmc_parameters['Dl_Poisson_150x150']['scale']
		Dl_Poisson_150x220 = data.mcmc_parameters['Dl_Poisson_150x220']['current']*data.mcmc_parameters['Dl_Poisson_150x220']['scale']
		Dl_Poisson_220x220 = data.mcmc_parameters['Dl_Poisson_220x220']['current']*data.mcmc_parameters['Dl_Poisson_220x220']['scale']
		
		TDust = data.mcmc_parameters['TDust']['current']*data.mcmc_parameters['TDust']['scale']
		ADust_TE_150 = data.mcmc_parameters['ADust_TE_150']['current']*data.mcmc_parameters['ADust_TE_150']['scale']
		BetaDust_TE = data.mcmc_parameters['BetaDust_TE']['current']*data.mcmc_parameters['BetaDust_TE']['scale']
		AlphaDust_TE = data.mcmc_parameters['AlphaDust_TE']['current']*data.mcmc_parameters['AlphaDust_TE']['scale']
		ADust_EE_150 = data.mcmc_parameters['ADust_EE_150']['current']*data.mcmc_parameters['ADust_EE_150']['scale']
		BetaDust_EE = data.mcmc_parameters['BetaDust_EE']['current']*data.mcmc_parameters['BetaDust_EE']['scale']
		AlphaDust_EE = data.mcmc_parameters['AlphaDust_EE']['current']*data.mcmc_parameters['AlphaDust_EE']['scale']
		
		MapTcal90 = data.mcmc_parameters['MapTcal90']['current']*data.mcmc_parameters['MapTcal90']['scale']
		MapTcal150 = data.mcmc_parameters['MapTcal150']['current']*data.mcmc_parameters['MapTcal150']['scale']
		MapTcal220 = data.mcmc_parameters['MapTcal220']['current']*data.mcmc_parameters['MapTcal220']['scale']
		MapPcal90 = data.mcmc_parameters['MapPcal90']['current']*data.mcmc_parameters['MapPcal90']['scale']
		MapPcal150 = data.mcmc_parameters['MapPcal150']['current']*data.mcmc_parameters['MapPcal150']['scale']
		MapPcal220 = data.mcmc_parameters['MapPcal220']['current']*data.mcmc_parameters['MapPcal220']['scale']
	
		cl = self.get_cl(cosmo)
		dlsEE = np.zeros(self.SPT3G_windows_lmax+1,'float64')
		dlsTE = np.zeros(self.SPT3G_windows_lmax+1,'float64')
		for i in range(self.SPT3G_windows_lmax+1):
			#print(int(self.ells[i]))
			dlsEE[i] = cl['ee'][int(self.ells[i])]*self.ells[i]*(self.ells[i]+1)/2./np.pi
			dlsTE[i] = cl['te'][int(self.ells[i])]*self.ells[i]*(self.ells[i]+1.)/2./np.pi
		#print("dlsEE=",dlsEE)
		#print("dlsTE=",dlsTE[0])
		...
		full_Dl_data_theory_binned = np.zeros(self.N_b_0*self.N_s_0,'float64')
		Dl_data_theory_difference = np.zeros(self.N_b_0*self.N_s_0,'float64')
		covariance = np.zeros((self.N_b_0*self.N_s_0,self.N_b_0*self.N_s_0),'float64')
		
		for i_spec in range(self.N_s_0):
		
			#specify EE
			if i_spec in [0,2,4,6,8,10]:
				#te_poisson_switch = 1
				current_iDust = ADust_EE_150
				current_iAlphaDust = AlphaDust_EE
				current_iBetaDust = BetaDust_EE
				dls = 1.0*dlsEE
				#print("i_spec=",i_spec,"dls=",dls)
			
				
			#specify TE
			if i_spec in [1,3,5,7,9,11]:
				#te_poisson_switch = 0
				current_iDust = ADust_TE_150
				current_iAlphaDust = AlphaDust_TE
				current_iBetaDust = BetaDust_TE
				dls = 1.0*dlsTE
				#print("i_spec=",i_spec,"dls=",dls)
				
			#print("i_spec=",i_spec,"dls=",dls)
			
			#Calibration for EE: 1/(Ecal_1*Ecal_2)
			#Calibration for TE: 0.5*(1/(Tcal_1*Ecal_2) + 1/(Tcal_2*Ecal_1))
			if i_spec==0:
				calibration = 1./( MapPcal90*MapPcal90 )
				te_poisson = Dl_Poisson_90x90
			elif i_spec==1:
				calibration = 0.5/( MapTcal90*MapPcal90 ) + 0.5/( MapTcal90*MapPcal90 )
				te_poisson = 0.
			elif i_spec==2:
				calibration = 1./( MapPcal90*MapPcal150 )
				te_poisson = Dl_Poisson_90x150
			elif i_spec==3:
				calibration = 0.5/( MapTcal90*MapPcal150 ) + 0.5/( MapTcal150*MapPcal90 )
				te_poisson = 0.
			elif i_spec==4:
				calibration = 1./( MapPcal90*MapPcal220 )
				te_poisson = Dl_Poisson_90x220
			elif i_spec==5:
				calibration = 0.5/( MapTcal90*MapPcal220 ) + 0.5/( MapTcal220*MapPcal90 )
				te_poisson = 0.
			elif i_spec==6:
				calibration = 1./( MapPcal150*MapPcal150 )
				te_poisson = Dl_Poisson_150x150
			elif i_spec==7:
				calibration = 0.5/( MapTcal150*MapPcal150 ) + 0.5/( MapTcal150*MapPcal150 )
				te_poisson = 0.
			elif i_spec==8:
				calibration = 1./( MapPcal150*MapPcal220 )
				te_poisson = Dl_Poisson_150x220
			elif i_spec==9:
				calibration = 0.5/( MapTcal150*MapPcal220 ) + 0.5/( MapTcal220*MapPcal150 )
				te_poisson = 0.
			elif i_spec==10:
				calibration = 1./( MapPcal220*MapPcal220 )
				te_poisson = Dl_Poisson_220x220
			elif i_spec==11:
				calibration = 0.5/( MapTcal220*MapPcal220 ) + 0.5/( MapTcal220*MapPcal220 )
				te_poisson = 0.
			else:
				raise io_mp.LikelihoodError("Cannot recognise field in spectrum")

			#Cl derivative
			#Convert to Cl
			cls = dls*2.*np.pi/(self.ells*(self.ells+1))
			#print("dls=",dls)
			
			Cl_derivative = np.zeros(self.SPT3G_windows_lmax+1,'float64')
			for i in range(1,self.SPT3G_windows_lmax):
				Cl_derivative[i] = 0.5*(cls[i+1] - cls[i-1])
			Cl_derivative[0] = Cl_derivative[1]
			Cl_derivative[self.SPT3G_windows_lmax] = Cl_derivative[self.SPT3G_windows_lmax-1]

			#Add CMB
			current_Dl_theory_unbinned = 1.0*dls[0:-1]
			#print(["%.12f" % x for x in current_Dl_theory_unbinned[499:600]])
			
			#Add super sample lensing
			#(In Cl space) SSL = -k/l^2 d/dln(l) (l^2Cl) = -k(l*dCl/dl + 2Cl)
			#l*dCl/dl
			super_sample_lensing = self.ells*Cl_derivative
			#Convert this part to Dl space
			super_sample_lensing *= self.ells*(self.ells+1)/2./np.pi
			#2Dl
			super_sample_lensing += 2.*dls
			#-kappa
			super_sample_lensing = -super_sample_lensing * Kappa
			#Switch on/off
			if self.SPT3G_Y1_EETE_super_sample_lensing==False:
				super_sample_lensing = 0.
			current_Dl_theory_unbinned += super_sample_lensing[0:-1]
			#print(["%.12f" % x for x in current_Dl_theory_unbinned[499:600]])
			
			#Aberration correction
			#AC = beta*l(l+1)dCl/dln(l)/(2pi)
			aberration_correction = -1.*self.SPT3G_Y1_EETE_aberration_coefficient*Cl_derivative*self.ells*self.ells*(self.ells+1.)/2./np.pi
			current_Dl_theory_unbinned += aberration_correction[0:-1]
			#print(["%.12f" % x for x in current_Dl_theory_unbinned[499:600]])
			
			#Simple poisson foregrounds
			simple_poisson = te_poisson*self.ells*(self.ells+1.)/(3000.*3001.)
			if self.SPT3G_Y1_EETE_poisson_sources==False:
				simple_poisson = 0.
			current_Dl_theory_unbinned += simple_poisson[0:-1]
			#print(["%.12f" % x for x in current_Dl_theory_unbinned[499:600]])
			
			#Polarised galactic dust
			#Amplitude is defined at l=3000 in Dl space, so we canvert to Cl power first and then to Dl
			dust = current_iDust*pow(self.ells/(80.),current_iAlphaDust+2)
			dust *= DustFreqScaling(current_iBetaDust,TDust,self.nu_0_dust,self.spectra_to_fit_nu_eff[i_spec,0])*DustFreqScaling(current_iBetaDust,TDust,self.nu_0_dust,self.spectra_to_fit_nu_eff[i_spec,1])
			if self.SPT3G_Y1_EETE_dust==False:
				dust = 0.
			current_Dl_theory_unbinned += dust[0:-1]
			#print(["%.12f" % x for x in current_Dl_theory_unbinned[499:600]])

			#Scale by calibration
			current_Dl_theory_unbinned *= calibration
			#print(["%.12f" % x for x in current_Dl_theory_unbinned[499:600]])
			
			#Now bin into bandpowers with the window functions
			current_Dl_theory_binned = np.dot(current_Dl_theory_unbinned,self.full_windows[:,self.bin_min:self.bin_max,i_spec])
			#Grab binned theory into long vector and calculate long difference
			for i in range(self.N_b_0):
				full_Dl_data_theory_binned[i_spec*self.N_b_0+i] = 1.0*current_Dl_theory_binned[i]
				Dl_data_theory_difference[i_spec*self.N_b_0+i] = current_Dl_theory_binned[i] - self.full_bandpowers[i,i_spec]
			#for i in range():
			#	print(["%.12f" % x for x in Dl_data_theory_difference])
			
		#Construct the full covariance matrix
		for i in range(self.N_b_0*self.N_s_0):
			for j in range(self.N_b_0*self.N_s_0):
				covariance[i,j] = self.beam_covariance[i,j]*full_Dl_data_theory_binned[i]*full_Dl_data_theory_binned[j]
		covariance += self.bdp_covariance
		#print("full_Dl_data_theory_binned=",full_Dl_data_theory_binned)
		
		#Get the contribution from the determinant part
		detcov = 0.5*np.log(np.linalg.det(covariance))
		
		#Calculate data likelihood
		SPT_LogLike = 0.5*np.dot(Dl_data_theory_difference, np.dot(np.linalg.inv(covariance), Dl_data_theory_difference)) + detcov
		
		#Calculate prior likelihood
		cal_vec = np.hstack([MapTcal90,MapTcal150,MapTcal220,MapPcal90,MapPcal150,MapPcal220])
		cal_vec = np.log(cal_vec)
		SPT_PriorLogLike = 0.
		if self.SPT3G_Y1_EETE_cal_prior==True:
			SPT_PriorLogLike = SPT_PriorLogLike + 0.5*np.dot(cal_vec, np.dot(self.cal_inv_covariance, cal_vec))
		#Add Gaussian prior for kappa
		if self.SPT3G_Y1_EETE_kappa_prior==True:
			SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((Kappa-self.SPT3G_Y1_EETE_Kappa_mean)/self.SPT3G_Y1_EETE_Kappa_sigma)**2
		#Add Gaussian prior for AlphaDust_EE
		if self.SPT3G_Y1_EETE_Alpha_Dust_EE_prior==True:
			SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((AlphaDust_EE-self.SPT3G_Y1_EETE_Alpha_Dust_EE_mean)/self.SPT3G_Y1_EETE_Alpha_Dust_EE_sigma)**2
		#Add Gaussian prior for AlphaDust_TE
		if self.SPT3G_Y1_EETE_Alpha_Dust_TE_prior==True:
			SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((AlphaDust_TE-self.SPT3G_Y1_EETE_Alpha_Dust_TE_mean)/self.SPT3G_Y1_EETE_Alpha_Dust_TE_sigma)**2
		
		#Add Gaussian prior for ADust_EE_150
		SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((ADust_EE_150-0.095)/0.012)**2
		#Add Gaussian prior for ADust_TE_150
		SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((ADust_TE_150-0.184)/0.072)**2
		
		#Add Gaussian prior for Dl_Poisson_90x90
		SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((Dl_Poisson_90x90-0.040469)/0.012141)**2
		#Add Gaussian prior for Dl_Poisson_90x150
		SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((Dl_Poisson_90x150-0.018048)/0.005414)**2
		#Add Gaussian prior for Dl_Poisson_90x220
		SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((Dl_Poisson_90x220-0.015719)/0.004716)**2
		#Add Gaussian prior for Dl_Poisson_150x150
		SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((Dl_Poisson_150x150-0.011495)/0.003448)**2
		#Add Gaussian prior for Dl_Poisson_150x220
		SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((Dl_Poisson_150x220-0.018962)/0.005689)**2
		#Add Gaussian prior for Dl_Poisson_220x220
		SPT_PriorLogLike = SPT_PriorLogLike + 0.5*((Dl_Poisson_220x220-0.047557)/0.014267)**2
		
		
		#print("SPT_LogLike=",SPT_LogLike)
		#print('detcov=',detcov)
		#print("SPT_PriorLogLike=",SPT_PriorLogLike)

		chi2 = SPT_LogLike + SPT_PriorLogLike
		lkl = - chi2
